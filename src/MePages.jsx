...
const NOTICES = [
  {
    ...
    title: '新店開幕，全場免運費',
    time: '今天',
    body: '珠珠客製新店開幕。即日起下單客製手鍊，全場免運費，把工坊裡的那條手鍊寄到你手上。',
  },
]
...
const FAQS = [
  {
    q: '點選珠子後為什麼會立刻上圈？',
    a: 'DIY 採用串珠流程：點選貨架照片，珠子會從卡片飛到圓環並自動排好。沿著圓周拖動可換位，拖出畫布可拿掉一顆。',
  },
  {
    q: '建議淨手腕圍要怎麼看？',
    a: '頂部會依目前珠串周長顯示建議淨手腕圍。實際佩戴可預留約 0.5cm 餘裕，太緊或太鬆都可在製作前調整尺寸。',
  },
  {
    q: '製作需要多久？',
    a: '送出製作後由人工選珠、穿串與覆核，一般 2–4 個工作天完成，遇國定假日可能順延。進度會同步到「我的通知」。',
  },
  {
    q: '售後要怎麼申請？',
    a: '收貨 7 日內如有斷線、缺珠或尺寸明顯不符，可點下方聯絡客服，並準備訂單編號與照片。客製款不接受無理由退貨。',
  },
  {
    q: '價格怎麼計算？',
    a: '每顆珠依所選直徑計價，畫布頂部會顯示合計。加減號改尺寸時價格會一起變。',
  },
]
...
const TERMS = [
  {
    title: '一、服務說明',
    text: '珠珠客製提供珠串搭配預覽與客製示範。頁面展示的價格、庫存與物流僅為示範數據，不構成真實交易要約。正式下單以實際平台協議為準。',
  },
  {
    title: '二、客製與交付',
    text: '手鍊依選定珠子與順序由人工製作。送出後進入製作流程，可在「我的訂單」查看狀態。因天然礦物存在色差、冰裂與紋理差異，成品與螢幕預覽可能略有不同，這屬於物料特性而非品質問題。',
  },
  {
    title: '三、個人資料',
    text: '收件地址、聯絡方式僅用於製作溝通與寄送。示範環境數據保存在本機，不會上傳。請勿在示範頁填寫真實敏感資料。',
  },
  {
    title: '四、智慧財產權',
  ...
  {
    title: '五、免責聲明',
    text: '因示範、網路或裝置原因導致的預覽中斷、資料遺失，平台不承擔額外賠償。礦石保養與佩戴風險請參考幫助中心說明。',
  },
]
...
export function DesignsPage({ designs = [], onDiy, onOpen, onDelete }) {
  if (!designs.length) {
    return (
      <Empty
        title="暫無設計"
        desc="在 DIY 畫布點「儲存設計」後，方案會出現在這裡。"
        action="去 DIY"
        onAction={onDiy}
      />
    )
    ...
    <>
      ...
      {editing && (
        <section className="me-card me-form">
          ...
          {[
            ['name', '收件人', '姓名'],
            ['phone', '手機號碼', '09 開頭的手機號碼'],
            ['region', '縣市鄉鎮', '例如：台北市大安區'],
            ['detail', '詳細地址', '路名、巷弄與門牌'],
...
export function TermsPage() {
  return (
    <article className="me-card me-article">
      <p className="me-article-lead">更新日期：2026 年 9 月 7 日 · 適用於珠珠客製示範與客製預覽。</p>
      {TERMS.map((block) => (
